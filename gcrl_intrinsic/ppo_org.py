...
def parse_args():
    ...
    parser.add_argument("--exp-name", type=str, default="cart_intr_"+os.path.basename(__file__).rstrip(".py"),
...
class RewardEngine(nn.Module):
    def __init__(self):
        ...
        self.state_predictor.load_state_dict(torch.load('state_predictor_cart.pt'))
    ...
    def get_reward(self, state, next_state, dones):
        ...
        pred_state = self.state_predictor(state)
        
        reward = F.mse_loss(pred_state, next_state[:,[0,2]], reduction='none').sum(dim=1)
...
if __name__ == "__main__":
    ...
    for update in range(1, num_updates + 1):
        ...
        # save agent 
        # torch.save(agent.state_dict(), "agent_cart.pt")
