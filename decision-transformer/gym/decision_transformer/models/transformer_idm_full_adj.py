...
class TransformerIDMFullADJ(TrajectoryModel):
    def __init__(self, state_dim, act_dim, max_ep_len, args) -> None:
        ...
        # self.embed_timestep_K = nn.Embedding(args.K, args.embed_dim)
    ...
    def forward(self, states, actions, rewards, returns_to_go, timesteps, attention_mask=None, tgt_mask=None):
        ...
        # time_embeddings = einops.repeat(self.embed_timestep_K.weight, 't d -> b t d', b=batch_size)
        ...
        representation = self.expander(x[:,0])

        return action_preds, representation
    ...
    def get_action(self, states, actions, rewards, returns_to_go, timesteps, **kwargs):
            ...
            # fixed timesteps 0 - seq_length
            # timesteps = torch.arange(0, timesteps.shape[1], device=timesteps.device).reshape(1, -1).repeat(timesteps.shape[0], 1)
