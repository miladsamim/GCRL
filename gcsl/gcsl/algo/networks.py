...
class StateGoalNetwork(nn.Module):
    def __init__(self, env, dim_out=1, state_embedding=None, goal_embedding=None, layers=[512, 512], max_horizon=None, freeze_embeddings=False, add_extra_conditioning=False, dropout=0):
        ...
        state_dim_in = env.observation_space.shape[0]#self.state_embedding(torch.tensor(torch.zeros(env.observation_space.shape)[None])).size()[1]
        goal_dim_in = env.goal_space.shape[0]#self.goal_embedding(torch.tensor(torch.zeros(env.goal_space.shape)[None])).size()[1]
